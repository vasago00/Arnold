...
// Pull the dailyLogs row for a date. Health Connect's syncDailyEnergy()
// populates totalCalories / activeCalories / steps on this row; if nothing
...
export function fuelAdequacy(dateStr) {
  ...
  // Nothing logged → pre-data fallback.
...
export function hrvBaseline(refDate, days) {
  ...
  const rows = storage.get('hrv') || [];
  const vals = windowedValues(rows, 'overnightHRV', ref, n);
...
function stageSubFromRow(row) {
  ...
  const score = Number(row.sleepScore);
  const hasStages = [deepMin, remMin, lightMin].every((v) => !isNaN(v) && v > 0);
  if (!hasStages) {
    // Fallback: sleepScore alone (DCY_SPEC §4.2 last paragraph).
    if (isNaN(score)) return { sub: null, hasStages: false };
  ...
  if (isNaN(score)) return { sub: stageSub, hasStages: true };
...
function latestFreshSleep(refDate) {
  const rows = storage.get('sleep') || [];
  if (rows.length === 0) return null;
  // Newest-first sort is cheap (rows are typically ≤ hundreds of nights).
  const sorted = [...rows].sort((a, b) => (b.date || '').localeCompare(a.date || ''));
  for (const r of sorted) {
    if (!r?.date) continue;
