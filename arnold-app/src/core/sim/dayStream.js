...
// Session templates: sampling weight + how they map to the engine's session shape.
// `intensityClass` matches the engine's HARD set (tempo/intervals/hiit/…) so eased
// sessions and fueling brackets are exercised. MET drives the kcal estimate.
const TEMPLATES = [
...
const LABEL = { rest: 'Rest', easy: 'Easy run', tempo: 'Tempo', intervals: 'Intervals', hiit: 'HIIT', strength: 'Strength', mobility: 'Mobility' };
const HARD = new Set(['tempo', 'intervals', 'hiit']);

function pickTemplate(rng) {
  let r = rng.uniform(0, TOTAL_W);
