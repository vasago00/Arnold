...
2026-07-26 (ROUND 99) — **DONE. The LIVING line stops borrowing a second engine's number.** "this week targets 19 mi" sat directly above a WEEK BUDGET strip reading 25. Both true, different weeks — and, the part that made it a bug rather than a wording problem, different ENGINES. The header now speaks only for the block, and says which week it means.

> Resume point — ROUND 99 is committed to the device, node-verified (26 harnesses green including the new `/tmp/wknum.mjs`, planSim 2,000 plans `ok:true` 0 hard violations, coachSim 5 seeds 0 violations, quality mean 0.9999). NOT yet built by Emil. Also carries the ROUND 98 field confirmation (see A99). Three follow-ups remain open and unstarted: **#146** (the Pick-your-road ladder prints five rungs that are four choices), **the provenance line**, and **#145** (coachContext / coachNarrative run-type Set). Emil chose the week-number fix over all three; #146 was the runner-up and is the natural next one.

**A99 — ROUND 98 CONFIRMED IN THE FIELD, and confirmed under version skew.** Emil built on Windows (full vitest suite green) and sent web + mobile screenshots taken at the same moment. Both screens now read PEAK 38 mi / CURRENT 3:58 / REACHABLE / +17.3 mi / base 20.7 / 19 wks / week budget 25. The 38-vs-43 and 3:58-vs-3:45 fork is **gone**.

The evidence is stronger than a plain match, because the two screens were running DIFFERENT CODE. Mobile had ROUND 98 (`PLAN PEAK`, "Needs 38 mi/wk at peak — what a 3:58 marathon demands. That is +17.3 mi/wk above what you are running now — the build."). Web was still serving a stale dev bundle from before it (`PEAK`, "Peak 38 mi/wk — what a 3:58 marathon needs. Building the base toward it — +17.3 mi to the peak."). Those old strings exist nowhere in `src/` — grepped, zero hits — and only one component renders that card, so it was a browser/vite cache, not a second implementation. Old code on one device and new code on the other producing the SAME numbers is `planPrefs` having actually converged, which is what `savePlanPrefs` was for. One observation, not a proof, but the right shape. **If a future session sees the web card looking "older", check for a stale bundle before diagnosing code.**

Note also that the `goal needs` stat did NOT render, which is correct: delivered peak 38 and demanded peak 38 differ by 0, and the ROUND 98 guard only prints the second tile when they differ by more than 2 mi. The feature working looks like nothing happening.

**B99 — THE BUG: a silent fallback ACROSS AN ENGINE BOUNDARY.** `LivingPlan.jsx` renders the LIVING line as `you're averaging 17.4 mi/wk · this week targets N mi · long M`. `N` came from `thisBlockWk`, a lookup that asks the generated block for the week whose `weekKey` equals the Monday of today. On Sunday 26 Jul that Monday is **20 Jul**; the block's first Monday is **27 Jul** (the plan starts tomorrow); nothing matched; `thisBlockWk` returned `null`; and the render fell through to `plan.targetWeeklyMiles`.

That fallback is the bug. `plan` is `coach.plan` = `seasonCoach.getSeasonCoach()` → `seasonPlan.resolveSeasonPlan()`, which computes its own single-week target as the 28-day average times a ramp (`seasonPlan.js:182-194`) **without ever looking at the block**. 25 is `planGenerator`'s periodized week 1 and is literally `sumRunMiles(days)` — the sum of the tiles below it. So the card printed one engine's number under words that promised the other's, in the one state where they were guaranteed to differ. Reproduced exactly in the harness: `seasonPlan → 19`, block week 1 → a different number, on identical inputs. 19 is the same 19 on Emil's screen.

This is the parallel-systems failure again, and in its nastiest form: not a fork between two surfaces, which a screenshot can catch, but a fallback between two engines behind one unchanged sentence, where there is no seam to see.

**C99 — THE RULE: THE BLOCK IS THE PLAN.** Three block states, three different sentences, and `seasonPlan` may never impersonate any of them:

- **covers this week** → unchanged: `· this week targets 25 mi · long 10`.
- **has not started yet** → name the date and quote the block's OWN first week: `· week 1 starts Jul 27 — it targets 25 mi · long 10`. Same number as the strip below, so the card cannot contradict itself.
- **ran out behind us** → print nothing. A finished block has no "this week", and back-filling one from a second engine is exactly how this bug happened.
- **no block at all** → `plan.targetWeeklyMiles` prints, and that is fine: it is then the only read on the card and there is nothing for it to contradict. An EMPTY `weeks` array counts as *no block*, not as a finished one.

The "not started" wording follows an idiom this same file already had at line ~1972 (`startDate > localDate() ? 'Week 1 · starts …' : 'This week'`) — the week-status strip had been getting this right all along, ten lines of JSX away from the header that was getting it wrong.

**D99 — ONE MONDAY.** `thisMondayKey` was lifted out of `thisBlockWk` into its own memo, because two callers now need it (the lookup, and the "has it started" test) and letting the second one compute its own would be the same class of bug in a smaller box. The harness asserts `(d.getDay() + 6) % 7` appears **exactly once** in `LivingPlan.jsx`. Keyed on `storageVersion`; `today` only moves at midnight, which is no worse than the old `[block]` key.

**E99 — NOT TAKEN, and why each is worth its own round.**

**#146 (NEW) — Pick your road prints five rungs that are four choices.** On Emil's card: Current 3:58 (38 mi), Target 3:45 (43), Stretch 3:43 (43), Goal 3:30 (48), Ceiling 3:20 (51). Target and Stretch are two minutes and zero miles apart. That is structural, not coincidence: `derive/raceOutlook.js:69-70` derives them as two improvement RATES over the same horizon, so a 19-week block converges them — and `:61` (`effTargetRate = IMPROVE_TARGET * (1 + frac * 0.8)`, commented "×1.8 at full promote ≈ STRETCH") deliberately bends Target toward Stretch under promotion. So when the athlete is being promoted they collapse **by design**. Emil: "The Pick your road on mobile is very busy for me in terms of design, there are 5 different situations." The app already owns the rule — the triad's *"three identical numbers are not a choice, they are noise pretending to be one"* (`LivingPlan.jsx` ~1771) and ROUND 98's >2 mi guard on `goal needs`. Applying it to the ladder (merge rungs whose demanded peak is within ~2 mi, keep the more conservative label) is 5→4 today and 5→3 on a short cycle. **Ships with the duplicate-control cleanup below.**

**Spread vs Run days — duplicated UI, NOT a fork.** Emil: "you have the Spread (which I like), but you also have the same in the planner." Checked: the Spread chips (`LivingPlan.jsx:1324`) and the Adjust-panel Run days input (`:2228`) both call the **same** `setRunDays` on the **same** `useState`. They cannot disagree. Still worth collapsing, and there is a live asymmetry while both stand: the chips offer 3–7, the input accepts 1–7. Recommendation: keep the chips (they sit beside the "N mi days" consequence, which is what makes them legible) and delete the input.

**The provenance line** — "Built from: Current · starts Mon 27 Jul · 19 wk to Valencia" — still unbuilt, still the highest-value diagnostic in the app. ROUND 98 and ROUND 99 are both the argument for it: each cost a round of forensics that one line on a screenshot would have made a five-second read.

**#145** — `coachContext.js:49` / `coachNarrative.js:36` still hold their own five-type `RUN_TYPES`, feeding gWeekDrift's mileage narrative. Unchanged from ROUND 98's F98. Needs the drift-narrative harness written FIRST, because that Set also drives `normType` fallback (`:57`) and session selection (`:109`), so widening it changes coach COPY and not just arithmetic.

**VERIFICATION (ROUND 99)** — `npx esbuild --loader:.jsx=jsx --bundle=false src/components/LivingPlan.jsx --outfile=/dev/null` clean. New harness `/tmp/wknum.mjs`, **31 assertions ALL GREEN**, structured as: (A) proves the two engines genuinely disagree on identical inputs, so the old fallback was a real fork and not a rounding — if they were ever equal the bug would have been invisible and the assertion says so; (B) the not-started state quotes week 1, from the block, never from seasonPlan; (C) the quoted number equals `sumRunMiles(week1.days)`, i.e. the tiles the athlete adds up by eye; (D) the covers-this-week state is unchanged; (E) a block behind us drops the clause instead of back-filling; (F) no block / empty weeks falls back honestly; (G) source assertions that the mirror cannot drift, that the literal `this week targets <b` is gone, and that exactly ONE Monday computation survives. Full sweep 26/26 harnesses green (triad 53, tierprog 108, adh, recal, trace 45, allweeks, rampbase, deliver, attrib, tf3, misses 24, modes 17, tracesync 22, profilesync 25, chips 4818, calread 1651, prefsync 72, coachproj 54, triadfwd 31, caps 1, proteinprobe 6, onesum 36, fmtfin 14, ctxtest 11, wr98 6, wknum 31). `runPlanSim` 2,000 plans → `ok: true`, HARD violations 0, aggregate violations [], acwr 99.84%, acwrMax 1.6, ramp 100.00%, peak 97.26% — byte-identical to the ROUND 98 baseline. `coachSim` 6,000 cases + seeds 1/42/777/20260101 × 3,000 → 0 violations. `qualityEval` → concise 1, grounded 1, timely 1, coherent 1, actionable 0.998–0.9991, mean 0.9999. No engine file was touched this round, which is why the sims are unchanged and why that is the expected result rather than a reassuring one.

**FILES this round —** `src/components/LivingPlan.jsx` (thisMondayKey extracted; thisBlockWk consumes it; the LIVING week clause rewritten into four states), `HANDOVER.md`, `COMMIT_MSG_99.txt`.

---


2026-07-26 (ROUND 98) — **DONE. One sum, one pref object, two differently-named peaks, one clock.** The plan that rendered 38 mi / REACHABLE / "3:58" on web and 43 mi / AGGRESSIVE / "3:45" on mobile at the same moment now cannot: the four places that computed a week's mileage were collapsed to one leaf module, the writer that silently deleted `tier` from the synced pref object was made impossible, and the two numbers both printed as "peak" were given different names.

> Resume point — ROUND 98 is committed to the device and fully verified in node (24 harnesses green, planSim 2,000 plans 0 hard violations, coachSim 5 seeds 0 violations, quality 0.9999). It has NOT been built by Emil yet. First thing next session: confirm the Windows build is green and the two screens now agree. Two follow-ups were opened and deliberately not taken: **#145** (coachContext / coachNarrative still hold their own run-type Set) and the **provenance line** on the plan card. Both are described below.

**A98 — what was actually wrong, in order of how much damage it did.** The headline finding was not an arithmetic bug. `storage.set` is a WHOLE-VALUE replace and `storage.merge` is array-only, so any writer handing `planPrefs` a partial object silently deleted every field it did not name. `PlanGeneratorPanel.jsx:58` and `SeasonPlanGenerator.jsx:98` each wrote exactly four fields — `availableDays, runDays, strengthDays, focus` — erasing the other eight: `target`, `targetExplicit`, **`tier`**, `tierExplicit`, `customGoalSecs`, `startDate`, `longRunDow`, `strengthDows`. `tier` is what selects `chosenRow` in LivingPlan, and `chosenRow` drives the goal time, the ramp, the peak the ramp delivers, and therefore the feasibility verdict. One deleted field, four divergent numbers — exactly the pair of screenshots. And because `planPrefs` is a synced key merged last-write-wins wholesale (`cloud-sync.js:631-636`), a truncating write did not just break its own device, it pushed the hole to the other one.

**B98 — the fix for it.** New `savePlanPrefs(patch)` in `core/storage.js`: reads the stored object, spreads the patch over it, writes the union. All three writers now go through it. The invariant is enforced by the harness, not by discipline — `/tmp/prefsync.mjs` now asserts that NO component contains `storage.set('planPrefs'` and that a four-field write leaves `tier`, `customGoalSecs` and `startDate` standing. Caveat recorded honestly: all three of those panels are orphaned (no imports anywhere in the tree), so the truncation explains a STALE SYNCED SNAPSHOT written while they were mounted, not a live per-render fork. A second live candidate is still untested — if the two devices are unpaired or have no cached passphrase, `planPrefs` never converges at all and both screens are correctly rendering two different plans. `savePlanPrefs` makes truncation structurally impossible either way; it may not alone close the symptom.

**C98 — the week-mileage sum, which turned out to be worse than the diagnosis said.** New zero-import leaf `core/runMiles.js` owns `SESSION_RUN_TYPES`, `isRunSession`, `sessionRunMiles`, `dayRunMilesRaw`, `sumRunMiles`. `hub/planGenerator.js` (`sumDayMiles`), `planTiers.js` (`budgetMi` and the WEEK BUDGET strip inside `weekBudgetStatus`) and `planner.js` (`dayRunMiles`, which now re-exports the Set and the sum rather than owning them) all point at it. The dead `_RUN_TYPES` at `MobileHome.jsx:2751` was deleted. But the *membership* of the set was itself a bug, and it only surfaced because the new harness asserted there was exactly one such Set in the tree and found ELEVEN. Six files were answering "does this session carry run miles" with six different answers: CalendarTab's `MILEAGE_TYPES` and LivingPlan's day tiles had all eight types; LivingPlan's `RUN_SET` (the "done" figure on the week card), `weekResolve`'s `RUN`, and coachContext/coachNarrative had five — no `race`, no `recovery`, no legacy `run`; and runMiles.js's first draft had five *different* ones — no `hiit`, no `recovery`, no `run`. So a HIIT session counted on the calendar and vanished from the week budget, and a race counted in the budget and vanished from the "done" number printed inches away on the same card. The widest set — CalendarTab's, the one the athlete adds up by eye — is now the shared one: `easy_run, long_run, tempo, intervals, hiit, recovery, run, race`. `CalendarTab.MILEAGE_TYPES`, `LivingPlan.RUN_SET` and `weekResolve.RUN` are now literally `= SESSION_RUN_TYPES`. **Note the direction of that fix: the first draft of runMiles.js would have NARROWED the app and silently dropped HIIT and recovery miles. The harness caught it, not review.**

**D98 — the two peaks now have two names.** `LivingPlan.jsx` printed a stat tile labelled `peak` carrying what the ramp DELIVERS (max `targetWeeklyMiles` across the generated block) six lines above a note reading "Peak N mi/wk — what a H:MM marathon needs" carrying what the finish time DEMANDS (`volumeModel.recommendedPeakMi`, a pure function of goal pace, blind to how many weeks are left). Two true numbers, one word. Now: the delivered tile is labelled **"plan peak"**, the demand appears beside it as a second tile labelled **"goal needs"** and ONLY when the two differ by more than 2 mi, and `volumeReadout.note` was reworded to `Needs N mi/wk at peak — what a H:MM marathon demands.` The trailing gap sentence, which read "+17.3 mi to the peak" and scanned as a shortfall in the plan, now reads "That is +17.3 mi/wk above what you are running now — the build", because that is what `gapMi` actually spans. `volumeModel.test.js` was tightened to assert the note does NOT open with "Peak ". `planTrace.js` already carried `wantsPeakMi`/`deliversPeakMi` correctly and `RaceOutlookCard`'s "asks / plan peaks" Fact already showed both — those were left alone.

**E98 — one clock (task #143).** `trainingProfile.js:79-87` held a SECOND `fmtFinish` that ROUNDED to the minute where `core/time.js` TRUNCATES — reopening, quietly, the exact one-minute fork that time.js's own header comment was written to end. It is deleted; trainingProfile now imports AND re-exports the shared one (the re-export matters: `trainingProfile.test.js:5` imports `fmtFinish` from there). Both pinned test values are exact multiples of 60, so truncate == round and the test passes untouched. The reason the duplicate existed was real, though: time.js had no sub-hour branch, so a 47-minute 10K rendered "0:47" and every caller with a non-marathon race had to grow its own copy — `RaceOutlookCard.jsx:42-47` had done exactly that, wrapping the shared formatter in an `if (s >= 3600)` guard with its own m:ss fallback. The sub-hour branch moved INTO `time.js` (seconds kept below the hour, truncated for the same two reasons the minutes are: never print a target easier than the one solved for, and `Math.round(59.6)` is a `:60`), so RaceOutlookCard's wrapper collapsed to an em-dash guard. `/tmp/fmtfin.mjs` sweeps the boundaries — 14 assertions, including 3:59:45 → "3:59" not "3:60", and 3599 → "59:59".

**F98 — NOT taken, on purpose.** (i) **#145** — `coachContext.js:49` and `coachNarrative.js:36` still hold their own five-type `RUN_TYPES`. `coachContext:213` uses it to build `runSessions` with per-session `mi`, which feeds gWeekDrift's "~36 mi against the ~42 target" — i.e. it IS a mileage question and it IS still forked. It was left because that same Set is also used at `:57` for the `normType` fallback and at `:109` for session selection, so widening it changes coach COPY, and the four coachContext tests were only just brought green. Do it as its own round with the drift-narrative harness in front of it. (ii) **The provenance line** on the plan card — "Built from: Stretch · starts Mon 27 Jul · 19 wk to Valencia" — remains the highest-value diagnostic in the app: one screenshot would attribute any future divergence instantly instead of costing a round of forensics. Still the recommended next build.

VERIFICATION — 24 node harnesses green (`triad tierprog adh recal trace allweeks rampbase deliver attrib tf3 misses modes tracesync profilesync chips calread prefsync coachproj triadfwd caps proteinprobe onesum fmtfin ctxtest`), including two new ones: `/tmp/onesum.mjs` (36 assertions — an adversarial week containing a two-a-day, a lift carrying a stray 3 mi and a 12-mi bike, proving the run-only rule gives 45.1 where the old naive sum gave 54.1; a source-level guard that no unaccounted run-type Set exists, with the genuinely-different-question Sets listed by name; and a two-device replay proving a partial write through `savePlanPrefs` preserves the plan while the same write through `storage.set` still forks it to exactly the 38-vs-43 pair off the screenshots) and `/tmp/fmtfin.mjs` (14). `/tmp/wr98.mjs` replays weekResolve's volume assertions under the widened set — conserved, and a recovery run's miles now count. `/tmp/prefsync.mjs` went 64→72 assertions. `runPlanSim` 2,000 plans: 0 hard violations, acwr 99.84%, ramp 100%, peak 97.26%. `runCoachSim` 6,000 cases + 4 extra seeds × 3,000: 0 violations. `runQualityEval`: concise/grounded/timely/coherent 1.0, actionable 0.9990, mean 0.9999. Every edited file passes `node --check` or esbuild. **`npx vitest run` remains unusable in this container (`Cannot find package 'vite'`) — the Windows build is the only real test run, and it has not happened yet for this round.**

FILES this round — NEW `src/core/runMiles.js`. EDITED `src/core/storage.js` (savePlanPrefs), `src/core/planner.js`, `src/core/hub/planGenerator.js`, `src/core/planTiers.js`, `src/core/time.js` (sub-hour branch), `src/core/trainingProfile.js` (duplicate fmtFinish deleted), `src/core/volumeModel.js`, `src/core/volumeModel.test.js`, `src/core/weekResolve.js`, `src/components/LivingPlan.jsx`, `src/components/CalendarTab.jsx`, `src/components/RaceOutlookCard.jsx`, `src/components/MobileHome.jsx`, `src/components/PlanGeneratorPanel.jsx`, `src/components/SeasonPlanGenerator.jsx`. Still true and still worth doing: `PlanGeneratorPanel.jsx`, `SeasonPlanGenerator.jsx` and `RecipePath.jsx` are orphaned — nothing imports them — and can be `git rm`'d; Cowork cannot delete files on the device.

---


2026-07-26 (ROUND 98 — PLAN / BRIEF, no code shipped yet) — **The next round is ONE problem: web and mobile render different numbers for the same plan at the same moment. This entry is the diagnosis, done in full so the next window starts by FIXING rather than by searching. Headline finding: it is NOT a "different component" bug — `LivingPlan` is a single component and `isMobile` only touches CSS — so the divergence is different INPUTS reaching identical code, and one input (`planPrefs.startDate`) re-defaults per device and plausibly explains three of the four divergences in a single chain. Also in this entry: the four failing `coachContext.test.js` assertions from ROUND 97 are fixed and node-proven (11/11).**

> Resume point for the NEXT window: **START HERE.** Read sections **A98–F98** below, then do fix **(i)** first (`planPrefs.startDate`), rebuild, and re-screenshot web + mobile "Your plan" side by side. If the peak/verdict/goal-time trio converges, **(ii)** the single-sum work is the rest of the round; if it does NOT converge, the fork is elsewhere and **(ii)** is where to look next. Do NOT start by rewriting the UI — every number on both screens comes from the same components; the bug is upstream in what those components are handed. **Also outstanding, mechanical:** `src/core/coachContext.test.js` is fixed in the working tree (four expectations moved to ROUND 96's intended behaviour, deliberately TIGHTENED, not loosened) — it needs to ride along in the next commit.

**A98) The symptom, stated exactly, from Emil's ROUND 97 screenshots.** One plan, one moment, two screens. Web "Your plan": PEAK **38 mi**, verdict **REACHABLE**, note *"what a 3:58 marathon needs — +17.3 mi to the peak"*. Mobile "Your plan": PEAK **43 mi**, verdict **AGGRESSIVE**, note *"what a 3:45 marathon needs — +22.3 mi to the peak"*. BOTH headers say *"this week targets 19 mi"* while the WEEK BUDGET strip on the same card says **26 mi**. Mobile EdgeIQ separately says *"ASKS / PLAN PEAKS 42 → 42 mi/wk"* and *"Stretch 3:46 · Just committed"* while the live stretch reads **3:43**. So there are four independently-divergent quantities: **(A) this week's target**, **(B) the peak**, **(C) the verdict**, **(D) the goal time**. Emil's framing is the right one and should stay the acceptance criterion: *"this is exactly the parallel systems computing the same thing differently problem."*

**B98) The finding that reframes the whole round: there is no second component.** The obvious hypothesis — web and mobile run different code — is FALSE, and ruling it out is what makes this tractable. `LivingPlan.jsx:201` is `export function LivingPlan({ …, isMobile = false, … })`; it is mounted exactly once, at `CalendarTab.jsx:1182`, with `isMobile` computed at `CalendarTab.jsx:437` from `window.innerWidth`. Every single use of `isMobile` inside LivingPlan (lines 1211, 1818, 1891, 1896, 1956, 1958) is layout — a class, a gap, a column count. **There is not one arithmetic branch on `isMobile` in the file.** EdgeIQ is the same story: the web path short-circuits to `MobileEdgeIQ` at `EdgeIQ.jsx:186-188` (`MobileEdgeIQ` is defined at `MobileHome.jsx:4183`), and both surfaces mount the SAME `RaceOutlookCard.jsx` (`EdgeIQ.jsx:673`, `MobileHome.jsx:4441`). **Conclusion: identical code, different inputs.** Anything the next round does to the rendering layer is wasted motion.

**C98) (A) "this week's target" is computed FOUR different ways, and only one of them understands `sessions[]`.** This one is device-independent — it is a plain defect and it explains "header says 19, strip says 26" on both screens at once. **(A1)** the card header at `LivingPlan.jsx:1069-1076` prints `Math.round(wkB.totalWeeklyMiles)`, which is written at `planGenerator.js:641` from **(A2)** `sumDayMiles` (`planGenerator.js:261`) — which sums the **top-level `d.distanceMi` of every day and never looks at `d.sessions[]`**. So a two-a-day counts ONCE and a cross-training day carrying a distance counts as running. `sumDayMiles` has exactly one caller and is not exported. **(A3)** when there is no generated block, the fallback `targetWeeklyMiles` comes from `seasonPlan.js:166-194` (phase multipliers 0.55/0.6/0.9/1.0 applied to `recentRunStats(...).weeklyMiles`) — a different quantity entirely. **(A4)** the WEEK BUDGET strip at `LivingPlan.jsx:1977-1996` renders `tw.budgetMi[k]` from a **re-implementation** of the same sum at `planTiers.js:342-345`, alongside `bs.totalMi` from a **third, inline** sum at `planTiers.js:445-479` (`num(t && t.distanceMi) || num(d.distanceMi)`, `BUDGET_TOLERANCE = 0.04` at `planTiers.js:418`) — and that third one depends on the **unpersisted `picks` state**, so it legitimately differs between two devices that have tapped different chips. **(A5)** `LivingPlan.jsx:1718` uses a fourth: `weekStatus.totals.runMiles` ← `weekPlanTotals` ← `dayRunMiles`. **(A6)** the canonical one, `planner.js:159-162`:
> ```js
...
> }
> ```
> `SESSION_RUN_TYPES` (`planner.js:124`) is `new Set(['easy_run','long_run','tempo','intervals','race'])` and is **module-private, not exported** — which is precisely why three other files rewrote the sum instead of importing it. It is also verbatim-duplicated as an UNUSED `_RUN_TYPES` at `MobileHome.jsx:2751`, which is the fossil of someone trying and giving up. **The seam is `makeDay` (`planner.js:173-179`): it writes only the PRIMARY session's miles to top-level `distanceMi`.** That single line is why A2 and A6 disagree. Existing callers of `dayRunMiles` to keep consistent: `MobileHome.jsx:21,2753`, `planAdherence.js:21,73`, `planLoad.js:6,44`, `coachContext.js:36,216-218` (imported, then `void dayRunMiles;`), `CalendarTab.jsx:28,996,1006,1046,1669,2043`.

**D98) (B) the peak is two conceptually DIFFERENT quantities wearing one label, plus a frozen third copy.** This is the 38-vs-43. **DELIVERS** — what the ramp actually reaches: `LivingPlan.jsx:966` `Math.max(1, ...block.weeks.map(w => w.targetWeeklyMiles || 0))`, and `planTiers.js:182-193` (`solveRampForPeak.measure`). Both measure `targetWeeklyMiles`, i.e. **excluding race miles**. **DEMANDS** — what the goal time is deemed to require: `volumeModel.js:24-30`, `const peak = 48 + (480 - paceSec) * 0.15;` clamped to [30,70]. Note what that is: **a pure function of goal finish time, with no input from training at all.** It flows to `tierFeasibility.js:257-272` as the row's `peakMi` and is rendered on the chips at `LivingPlan.jsx:1243`. **FROZEN** — `planTrace.js:104-126` reads `c.peakMi` / `c.deliversPeakMi` straight off the `planCommitment` store key and *explicitly does not recompute*; `planCommitment.js`'s own comment records *"at Emil's July 2026 numbers, 44 vs 35"*, i.e. the wants/delivers gap was known and written down at the time. `RaceOutlookCard.jsx:359` calls `planTrace({ aRaceDate })`, which is how a stale committed number reaches EdgeIQ ("PLAN PEAKS 42 → 42"). **38 has the shape of a DELIVERED peak; 43 has the shape of a DEMANDED or FROZEN one.** Ignore `SeasonPlanGenerator.jsx:199,203,213,215` — that component is never mounted.

**E98) (C) the verdict escalates on the CALENDAR, not on the mileage — and there are two unrelated enums both spelling "aggressive".** `tierFeasibility.js:59` sets `RATIO_BANDS = { comfortable: 1.15, reachable: 1.35, aggressive: 1.50 }` and `:213` computes `const ratio = held > 0 ? req.peakMi / held : Infinity;`. Then `:244-250`:
> ```js
...
> ```
> **One week's difference in block start flips REACHABLE → AGGRESSIVE with no change in mileage whatsoever.** `recommendedTier` (`:290`) then filters on that same verdict, so the escalation also changes which tier is recommended. **Trap for the next window:** `seasonPlan.js:68-89` returns a SECOND, unrelated enum that also contains `'aggressive'`, and it is rendered on the same card line (`LivingPlan.jsx:963`, `FEAS_COLOR`). Seeing "AGGRESSIVE" on screen does NOT tell you which engine produced it — check which one before chasing it.

**F98) (D) the goal time has two variables and a surviving duplicate formatter.** `LivingPlan.jsx:542-546` builds `goalVol` from `buildGoalSecs` (assigned at `:536` as `chosenRow.goalSecs`, but only on the chosen-row branch of `483-540`), and renders it at `:1124-1131` as `◎ {goalVol.note}`, where the note text is `volumeModel.js:64` `` `Peak ${peakMi} mi/wk — what a ${goalStr} marathon needs.` ``. The chip an inch away renders `fmtGoal(r.goalSecs)` at `:1240`. **Two different variables through the same formatter, side by side** — that is 3:58 next to 3:45. The ladder times themselves come from `raceOutlook.js:37-76` (`IMPROVE_TARGET = 0.025`, `IMPROVE_STRETCH = 0.045`, `MAX_CYCLE_VDOT_GAIN = 4`, `yrs = yearsBetween(today, race.date)` — so **a different `today` slides Stretch continuously**, which is the 3:46-vs-3:43). EdgeIQ's Stretch arrives by a different path: `raceOutlookLive.js:114` via `RaceOutlookCard.jsx:79-105`. Separately and smaller: `trainingProfile.js:79-87` still defines a SECOND `fmtFinish` that **ROUNDS** where `core/time.js:137-166` **TRUNCATES** — worth exactly one minute, so it explains 3:46 vs 3:45 but NOT 3:58 vs 3:45. It leaks out inside the profile object rather than by direct import, which is why grepping for the import found nothing.

**G98) THE PRIME MOVER — one fork that explains B, C and D together: `planPrefs.startDate` re-defaults per device.** `LivingPlan.jsx:228`:
> ```js
> const [startDate, setStartDate] = useState(() =>
>   (init.startDate && init.startDate >= localDate()) ? init.startDate : localDate());
> ```
> and the component writes it straight back at `:719` (`storage.set('planPrefs', { …, startDate, longRunDow, strengthDows }, { skipValidation: true })`). So a device whose stored `startDate` has gone past silently substitutes ITS OWN today — and two devices opened on different days, or with different stored prefs, start from different dates. `storage.js:266-289` already documents this exact failure in a comment: *"a plan that starts 27 Jul is 19 weeks to Valencia while one that starts a week earlier is 20. Different weeksToRace solves a different ramp (9.3–12.9 %/wk vs 7.0–10.1), which peaks at a different mileage (39 vs 41), which makes a different tier reachable."* The `migratePlanPrefsKey` fix (`storage.js:583-603`, called once at `Arnold.jsx:457`) is a **one-time, flag-guarded key move — it does not stop per-device re-defaulting.** The chain: different `startDate` → different `today` into the generator (`LivingPlan.jsx:564`, `const base = { races, today: startDate, … }`) → different `weeksToRace` → different ramp → **(D98)** different delivered peak → **(E98)** the `tierFeasibility.js:244-250` escalation trips on one device and not the other → different `chosenRow` → **(F98)** different `buildGoalSecs`. **One fork, three symptoms.** (A98) is separate and device-independent.

**H98) The four fixes, in the order they should be done.** **(i)** Stop `startDate` re-defaulting per device — either persist the ORIGINAL start and let it go into the past, or keep the display default but exclude it from the ramp's `today`. This is the one to do first because if the hypothesis holds it collapses three symptoms at once, and because every other fix is harder to evaluate while the inputs still fork. **(ii)** Make `LivingPlan.jsx:1069`, `weekBudgetStatus` (`planTiers.js:342-345`), the inline sum at `planTiers.js:445-479` and `mergeTriadWeeks` share ONE sum — it should be `dayRunMiles`, because it is the only one that understands `sessions[]`; that means **exporting `SESSION_RUN_TYPES` from `planner.js:124`** and deleting the dead `_RUN_TYPES` twin at `MobileHome.jsx:2751`. **(iii)** Label B-delivers and B-demands DISTINCTLY on screen — "your ramp reaches 38" vs "a 3:45 wants 43" — rather than printing both as "peak"; the gap is real information and hiding it behind one word is what makes the two screens look broken rather than informative. **(iv)** Delete `trainingProfile.js:79-87`'s `fmtFinish` in favour of `core/time.js`. **Acceptance:** web and mobile, opened at the same moment, show the same peak, the same verdict, the same goal time, and a header target that equals the week-budget strip.

**VERIFICATION (this entry).** No product code shipped this round. `src/core/coachContext.test.js` was corrected for the four ROUND 97 failures Emil pasted and PROVEN rather than guessed: `/tmp/ctxtest.mjs` replays all four vitest cases under plain node (11 assertions, rebuilding the file's `day()` and `mkCtx()` helpers and importing `computePlanSlice` from `coachContext.js` and `allBeats` from `coachNarrative.js`) and returns ALL GREEN — 11 passed, 0 failed. `node --check` clean. The expectations were TIGHTENED, not loosened: `missed` entries now assert the `date` field ROUND 96 added, and the two drift leads assert the WEEKDAY NAME (`Tuesday's easy run`, `Saturday's long run`) rather than being relaxed to `.*` — the day name is the whole point of the ROUND 96 change, and asserting it also pins the date parsing against the `new Date(dateStr)` local-time drift class of bug that is still open in `tierFeasibility.js`. The diagnosis in A98–G98 is a read-only source survey (line numbers as of this commit); nothing in it has been executed.

**FILES this round —** `src/core/coachContext.test.js` (four expectations moved), `HANDOVER.md` (this entry).

---


2026-07-26 (ROUND 97) — **Six complaints from four mobile/web screenshots, all shipped: two strips that could not fit on a phone are now one row that can never wrap, a Fuel comment that told Emil to eat 40g of protein at 00:53 on a rest day with an empty food log is gated three ways, and Arnold's writing rule ("capitalise the first or only word") is applied across 26 components AND pinned by a new source-scanning vitest guard so it cannot drift back. Build-blind; node-verified by 21 harnesses, the 6,000-case coach sim across 5 seeds, the 4,000-case quality eval and the 2,000-plan population sim — 0 failures.**

> Resume point for the NEXT window: **(1)** Emil rebuilds and screenshots mobile EdgeIQ (does the 5-bin tier row fit ONE line with CEILING intact?), mobile Calendar → Your plan (does the PICK YOUR ROAD chip grid render as 3×2 with nothing dangling?), web Your plan (is `Working ▾` on the same line as the chips?) and the Fuel card AT NIGHT (the protein beat must be SILENT before 06:00 and after 21:00). ROUND 97 is NOT build-verified. **(2) STILL UNRESOLVED and now WORSE than ROUND 96 recorded it — the same plan shows DIFFERENT numbers on web and mobile simultaneously:** web "Your plan" says PEAK **38 mi** / verdict **REACHABLE** / "what a 3:58 marathon needs — +17.3 mi to the peak"; mobile "Your plan" says PEAK **43 mi** / **AGGRESSIVE** / "what a 3:45 marathon needs — +22.3 mi to the peak"; both headers say "this week targets 19 mi" while the WEEK BUDGET strip says 26 mi; mobile EdgeIQ separately says "ASKS / PLAN PEAKS 42 → 42 mi/wk" and "Stretch 3:46 · Just committed" while the live stretch reads 3:43. That is at least four derivations of one week. Candidate mechanism unchanged: `sumDayMiles` (planGenerator) sums ALL `distanceMi` while `dayRunMiles` (planner.js:159) filters by `SESSION_RUN_TYPES`. **Pick ONE derivation and make every other surface read it — this is now the largest open correctness item and it is exactly the "parallel systems computing the same thing differently" class Emil has named as his main fear.** **(3)** The `type`/`label` divergence in stored planner days is still patched only at the NARRATION layer; provenance unchased (`relabelSeason` at planGenerator.js:343 rewrites `label` from `PLAN_LABEL[d.type]`, so the divergence comes from older-generator data or `CalendarTab`'s hand-edit path). **(4)** `toISOString` timezone bug still live in `core/tierFeasibility.js` (~68–72, ~114) and `LivingPlan.jsx`'s session drill-down. **(5)** Orphans to delete: `RecipePath.jsx`, `SeasonPlanGenerator.jsx`, `PlanGeneratorPanel.jsx` — both generator panels write a PARTIAL four-field `planPrefs`. **(6)** Web nav labels still swapped. **(7)** `goals.weeklyRunDistanceTarget || 30` still powers ~12 dashboard tiles nothing writes back to. **(8)** Reach-rung steepness (`RUNG_ACWR.reach = 1.19`) awaiting Emil's call.

**A) #140 — the Fuel comment, and why all three of its clauses were wrong at once.** Emil, at 00:53 local, on a day with nothing run and nothing eaten: *"what kind of comment is this that I am short 153g of protein? This is not rational and timed well."* The beat was `gMechanism` in `coachNarrative.js`, and it fired on a single condition — `primarySession.type` is `strength` or `long_run` and the protein gap exceeds 5g. Every part of what it then said was false. It claimed **"after the long run"** about a session that had not happened, because `primarySession` is what the plan WROTE for today, not what was done. It prescribed **"~40g in the next couple hours (and before bed)"** at nearly one in the morning, which is not advice, it is an alarm clock. And it computed a **153g shortfall** from an empty food log, where the "gap" is just the target restated — a fact about the LOG, not about the athlete. Three defects, one symptom, so three separate gates rather than one broad suppression: `day.postWorkout` (the MPS window has to EXIST), `day.fuelWindowOpen` (the world model's own 06:00–21:00 eating-hours gate; an unknown clock leaves it CLOSED, so the beat stays silent rather than guessing — the same no-fabrication stance as the rest of the file), and a `protein.today` presence check that is the exact twin of `gEnergyAvailability`'s existing guard 20 lines below.

**B) The beat was not deleted, because the nudge is real — it just has to say only what it knows.** Dropping `mech-protein-timing` on an empty log would have thrown away a genuinely useful post-session refuel prompt to fix a wording bug. Instead there are now two wordings, chosen on whether anything has been logged: the original gap-based sentence when there is intake to reason about, and, when the log is empty, *"Nothing logged since the long run yet. Don't chase today's 153g total — get ~40g in over the next couple of hours: that first dose is what turns the session into muscle rather than just a deficit."* The dose and the target both ride in `claim.data`, so `factCheck` grounds them and the quality eval's `grounded` dimension stays at 1.0.

**C) Why it survived for months: the eval had no rule for it, and the sim could not reach the state.** Two independent holes, both closed. `coachQuality.js`'s `TIME_SENSITIVE` map had entries for `reds-lowEA` and `fuel-status` and **nothing for `mech-protein-timing`** — `timely: 1.0` was true and meaningless for this beat. It now has `(day) => !day.postWorkout || !day.fuelWindowOpen`, and `/tmp/proteinprobe.mjs` proves the rule has TEETH by asserting `assessTimely` FAILS at 01:00/no-session and PASSES at 18:00/trained (a green eval alone cannot distinguish "the rule works" from "the rule is vacuous"). Separately, `coachNarrativeSim.js` drew intake as `rng.int(0, 3200)` and `rng.int(0, 180)`, which makes **"exactly nothing logged" a ~0.5% coincidence** — the no-data branch of every fuel beat was being graded on effectively zero cases, and it is precisely the state Emil screenshotted (0 of 1,880 kcal · 0 of 153g). The empty food log is now its own modelled outcome: `rng.chance(0.15)`, with BOTH numbers zero together the way they are in real life. **This is the second round running in which a green sim was green over a distribution that could not reach the code under test** (ROUND 96's was `donePlannedMi: 0 of 6,000`). Treat "the sim is green" as a claim about the sim's distribution until the distribution is checked.

**D) #141 — "These buttons are too big, they should fit in one row," and the identical bug in the other strip.** `RaceOutlookCard`'s tier ladder used `repeat(auto-fit, minmax(84px, 1fr))`, which needs 5 × 84 + gaps ≈ 440px; a 390px phone cannot supply it, so the row wrapped to 3 + 2 and clipped CEILING at the right edge. `LivingPlan`'s PICK YOUR ROAD chips had the mirror-image failure — six chips of unequal width flowing into a wrapping row gave Emil a first line ending mid-option, which reads as truncation and is not wrong, because nothing visible says where the ladder ends (*"This does not display fully or well"*). Both are fixed the same way and it is deliberately ONE technique, not two: an explicit `repeat(N, minmax(0, 1fr))` **can never wrap** — the bins divide whatever width exists — plus `clamp()` typography so the same declaration is 13.7px at 390px and 15.5px on web, plus a `maxWidth` so the bins cannot balloon back into the ~200px dead-space boxes Emil complained about two rounds ago on a 1400px card. No media queries and no second layout to keep in sync. The chips get `display: contents` on web and a 3-column grid on mobile, which is the same markup serving both surfaces rather than a phone branch that can drift.

**E) #142 — the web strip, and the 90px of duplication that was pushing `Working ▾` onto its own line.** Emil: *"this Pick your road container is too big for the information it holds and it drops to a new line for the working sign."* Measured rather than guessed: the strip wanted ~1423px inside a ~1318px content box. The single biggest saving was not styling — it was **deleting `asks 38`**, which printed the selected option's peak a SECOND time about 5cm to the right of the chip already showing that exact number. `peaks N` survives but only when the ramp genuinely falls short (`shortfallMi > 2`), because that is the only state in which it says something the chip does not. With the chip padding/letter-spacing tightened and the applied-state chip shortened to `✓ N wks · Label`, the strip lands near ~1253px. **The general lesson is worth keeping: when a row overflows, look first for a number printed twice, not for a font to shrink.**

**F) #143 — "All writing in Arnold should start with a capital letter if this the first or the only word," and why this became a TEST.** Emil named a dozen offenders he could see (`of 1,880`, `of 153g`, `rest day`, `just committed`, `all three roads · 6 mi`, `working ▾`, `base 20.7`, `your own`, `checkpoint — prove it here`). Grepping found **~130 across 26 files**. That is not a bug, it is DRIFT: every round adds a label, and a convention nobody checks decays to whatever each site felt like at the time — so a hand-sweep fixes today's screenshots and guarantees the complaint returns. The sweep was therefore done MECHANICALLY (85 label-slot strings by regex with a protected allowlist, 42 JSX text nodes by hand-verified pair list, 9 attribute strings) and then **pinned by a new source-scanning vitest guard, `src/components/ui/sentenceCaseGuard.test.js`**, modelled on the existing `buttonHeightGuard.test.js`. It scans label-ish object keys and `placeholder`/`aria-label`/`title` attributes and fails CI on any lowercase first word. Bare JSX text nodes are deliberately OUT of the guard's scope: `<span>{label}</span><span>or click</span>` is a legitimate mid-sentence continuation, and a guard that cannot tell prose from a label would train people to add exceptions instead of fixing strings. The `ALLOW` set is short and holds only things whose lowercase spelling is CORRECT — units and assay names (`mL`, `hsCRP`, `rTSS`, `kcal`, `lb`, `mi`), input format tokens (`hh`, `mm`, `ss`). Capitalising those would be a different error, not a fix.

**G) One label was deleted rather than fixed, and the races were tightened.** `GOAL = TARGET` rendered as `GOAL = TA…` because six bins on one phone row give each label ~45px and that string wants ~70px — but it was also buying nothing, since `CoincideLine` sits directly beneath and states the same fact as a whole sentence; it is now just `Goal`. Likewise `● committed` became `Committed`: the selection ring IS the bullet, and dropping the glyph bought the label the ~7px it needed to survive six-across without an ellipsis. For *"the races and times should be tighter together"* the race chips keep the same box and lose the air inside it — padding 7/9 → 4/8, the meta line rides 1px under the name instead of 2, and the grid gap drops 6 → 3 — so six stacked chips no longer sit ~46px apart centre-to-centre for two short lines of text.

VERIFICATION (all node, no vitest in the Cowork container): `node --check` clean on `coachNarrative.js`, `coachQuality.js`, `coachNarrativeSim.js`, `sentenceCaseGuard.test.js`; esbuild clean on all 26 touched components. **21 harnesses green** — triad(53) tierprog(108) adh recal trace(45) allweeks rampbase deliver attrib tf3 misses(24) modes(17) tracesync(22) profilesync(25) chips(4818) calread(1651) prefsync(65) coachproj(54) triadfwd(31) **caps(NEW)** **proteinprobe(NEW, 6)**. `runPlanSim` 2,000 plans: 0 hard violations, acwr 99.84% / ramp 100% / peak 97.26%. `runCoachSim`: 0 violations at seed 20260716 × 6000 and at seeds 1/42/777/20260101 × 3000 — re-run AFTER the sim widening, so the 0 is over a distribution that now actually reaches the empty-log branches. `runQualityEval`: grounded/timely/coherent/concise = 1.0, actionable ≥ 0.998, mean ≥ 0.9998 across seeds 1/42/777; the only fails are the pre-existing `gentle read has no action cue` on strength-progress beats, **none involving the new branches**.

FILES this round — `src/core/coachNarrative.js` (gMechanism: three gates + zero-logged wording), `src/core/sim/coachQuality.js` (TIME_SENSITIVE gains `mech-protein-timing`), `src/core/sim/coachNarrativeSim.js` (`emptyLog` outcome at 15%), `src/components/RaceOutlookCard.jsx` (one-row ladder, tightened race chips, capitalised STATUS/VERDICT maps), `src/components/LivingPlan.jsx` (mobile 3-col chip grid, web width recovery, `asks` removed), **NEW `src/components/ui/sentenceCaseGuard.test.js`**, plus capitalisation-only edits to `AddedLoad · CalendarTab · ChangeSessionWindow · CloudSyncPanel · CockpitRail · EdgeIQ · GoalsHub · HubCard · LabsModule · LearnedHero · LogDay · MobileHome · NutritionInput · PlannedWorkoutTile · PostRunWeigh · RaceFocusCard · RecipePath · RecordPanel · RedsCard · SwapImpactModal · TrainingTab · WebSystemDetail · WeeklyPlanner · Workbench`.

---


2026-07-26 (ROUND 96) — **The two things Emil said were still broken after the ROUND 95 build, both root-caused to something deeper than the symptom: the calendar tiles were blank because the tier freeze is written ONCE and never re-cut (so no rebuild could ever fix them), and the coach was still naming a session `easy_run` on a day his own calendar calls "Intervals 5mi" — his planner's `type` and `label` disagree in real stored data. Fixed by re-cutting the triad FORWARD on every living refresh (no re-apply, ever) and by making the coach name sessions the way the athlete was shown them and decompose the week's gap instead of blaming it all on the miss. Build-blind; node-verified by 19 harnesses (7,000+ assertions), the 6,000-case coach sim across 5 seeds, the 4,000-case quality eval and the 2,000-plan population sim — 0 failures.**

> Resume point for the NEXT window: **(1)** Emil rebuilds and screenshots the month grid (do the tier rows appear on THIS week's tiles, without touching the plan?) and the coach paragraph on Daily/Play and Calendar → Your plan. ROUND 96 is NOT build-verified. **(2) STILL UNRESOLVED — three different numbers for the SAME week on screen at once:** LIVING header "this week targets 17 mi" (`thisBlockWk.totalWeeklyMiles`, `LivingPlan.jsx` ~1035–1059), coach "~21 mi target" (planner week), week strip "26 mi" (block week 1). A concrete candidate mechanism was found this round: `sumDayMiles` (planGenerator) sums ALL `distanceMi` while `dayRunMiles` (planner.js:159) filters by `SESSION_RUN_TYPES`, so a `type:'cycle'` day carrying `distanceMi: 5` is counted by one and not the other. Pick ONE derivation and make the other two read it. **(3) NEW — the `type`/`label` divergence in stored planner days is now patched over at the NARRATION layer, but its PROVENANCE is unchased.** `relabelSeason` (planGenerator.js:343) runs last in the generator week loop and rewrites `label` from `PLAN_LABEL[d.type]`, so freshly generated days always agree; the divergence therefore comes from older-generator data or from `CalendarTab`'s hand-edit path (`makeDay`/`savePlannerWeek`). Until that is found, any NEW surface that reads `type` will be wrong again. **(4)** `toISOString` timezone bug still live in `core/tierFeasibility.js` (~68–72, ~114) and `LivingPlan.jsx`'s session drill-down. **(5)** Orphans to delete: `RecipePath.jsx`, `SeasonPlanGenerator.jsx`, `PlanGeneratorPanel.jsx` — both generator panels write a PARTIAL four-field `planPrefs`. **(6)** Web nav labels still swapped. **(7)** `goals.weeklyRunDistanceTarget || 30` still powers ~12 dashboard tiles nothing writes back to. **(8)** Reach-rung steepness (`RUNG_ACWR.reach = 1.19`) awaiting Emil's call.

**A) #137 — "I do not see anything on the daily tabs on the calendar. I expect where the 3 lines are I see the different distance depending on what I run."** ROUND 95 removed the spread gate from `packTriad`, which was necessary and not sufficient. THE FREEZE IS FROZEN: `planCommitment.triad` is written exactly once, at apply time, by `commitTier`. Rebuilding the app re-runs the generator but never re-packs the record, so every calendar tile kept reading the SAME v1 blob that had already dropped his flat weeks. No rebuild could ever have fixed this, and the only user-facing remedy was "re-apply your plan" — which is precisely the sentence Emil pre-rejected: *"I do not want to touch the plan again, if it is truly a living plan."*

**B) The fix — `refreshTriadForward`, and why the line is TODAY.** New pure function in `core/planTiers.js`: given the frozen record, a freshly packed one, and today's date, it keeps every day strictly BEFORE today from the frozen record and takes today-and-forward from the fresh one, per week, per day index. `LivingPlan.jsx` calls it from inside the LIVING auto-apply block (the one the mount `useEffect` at ~813 already fires) via a new `refreshCommittedTriad`, which writes only when the merge actually differs — so it is idempotent and cannot churn storage. The boundary is TODAY rather than the week edge because that is exactly where `pasteSeasonBlock` already draws its own line in `'refresh'` mode: LivingPlan re-baselines FORWARD days on every living refresh, so if the triad were re-cut on any OTHER beat it would immediately become a second system computing the same week differently — the failure class Emil has named as his main fear. This is simultaneously the MIGRATION (a v1 record self-heals today-forward with no user action) and the removal of a latent parallel-systems bug that predates the blank tiles. `TRIAD_PACK_V` went 1 → 2 so a stale record is identifiable rather than merely wrong.

**C) The option that was rejected, and why.** Having `CalendarTab` reconstruct a missing day's triad from the planner's distance was the smaller diff and the wrong answer twice over: it breaks the #130 rule that the calendar NEVER builds a triad (it only reads the freeze back), and the old packer dropped days for TWO indistinguishable reasons — flat spread AND non-monotone rungs — so reconstruction would invent a clean ladder on days where a real 7/6/7 row existed, and then grade his run against a ladder that never existed. Re-cutting from the same freshly generated block the rest of the plan is already being re-baselined from cannot disagree with anything, because it IS the same source.

**D) #138 — "the Coach is still saying that I missed the easy run."** Two distinct wrongs hiding behind one sentence, and ROUND 95 had only fixed a third. **NAMING:** the coach named a session by `type`; every screen the athlete has ever looked at names it by `label`. Reading his ACTUAL export proved the two disagree in his stored data — `2026-07-16` is `{type:'easy_run', label:'Intervals 5mi'}` and `2026-07-15` is `{type:'cycle', label:'Tempo 5mi + strength'}`. So the coach said "you didn't get the easy run" about a day his calendar labels *Intervals*. It was not a lie and it was not a rendering bug; it was two names for one thing, and the athlete's name is the one that counts. `computePlanSlice` now carries `label` and `date` on every missed/remaining entry, and `nameOf`/`dayNameOf` in `coachNarrative.js` turn them into **"You didn't get Thursday's intervals"**. When the metadata is absent the phrasing is byte-for-byte what it always was — load-bearing, because `coachContext.test.js` and the 4-arg `computePlanSlice` callers depend on it.

**E) The other half of #138 — the coach had the CAUSE wrong, and that is worse than having it long.** The sentence was `${lead}, so ${tail}` unconditionally: a causal claim that the skipped session produced the entire shortfall. Emil's own tile reads **7.5 / 9.0 mi** — he also ran short on days he DID get out, so the claim is simply false, and a coach that gets the cause wrong cannot be trusted on the remedy. `computePlanSlice` now reports `donePlannedMi` (what the days he ran had asked for) alongside `weekMiActualRaw`, which makes the gap decompose exactly: with every planned session landing in exactly one of {done, done-unmeasured, missed, remaining}, `weekMiTarget = donePlannedMi + doneUnmeasuredMi + missedMi + plannedLeftMi`, therefore `gap = missedMi − (actual − donePlannedMi)`. `gWeekDrift` now says *"You didn't get Thursday's intervals, and you were ~2 mi short across the days you did run — together the week's tracking…"*, or, when he ran MORE than asked, *"…which covers part of it, so…"*, and only asserts the bare `so` when the arithmetic actually supports it. Raw unrounded values are differenced deliberately: subtracting two rounded numbers is where a phantom "1 mi short" is born.

**F) Two holes found and closed en route, neither of them reported.** (i) A run logged with **no distance** (a manual entry; a watch that synced the session and not the track) put the date in `runOn` but not in `runMiOn`, so it counted as ZERO miles against its full prescription and manufactured a 9-mile shortfall out of a run he actually did — the same species of lie ROUND 95 existed to kill. New `doneUnmeasuredMi` credits the prescribed distance (the least-inventing assumption available), keeps the amount separate, and **gates every short/extra claim**: if any part of the week is unmeasured the coach makes NO claim about running short. (ii) `gWeekDrift` returned `null` the instant `missed` was empty, so the athlete who gets out every single day and quietly runs 7.5 where the plan wrote 9 had a completely silent coach — the drift beat inverted: loud about distribution, deaf to delivery. New **`week-short`** beat credits the showing-up first, gives the number, and offers the two honest exits (put the miles on an easy day, not the long run; or say the written numbers are too long and let the plan come down to meet you). Thresholds `gap ≥ 2 mi` and `short ≥ 2 mi` with a ±0.5 deadband keep it from becoming a nagging machine.

**G) The sim was extended so none of this is green-by-absence.** `coachNarrativeSim.js` previously synthesised `weekMiActual` but nothing else, so `donePlannedMi` was set in **0 of 6,000** generated contexts — every new branch was unreachable and the sim's 0 violations meant nothing about them. It now generates the full decomposition obeying the same identity the real slice does, gives half the missed entries a real `label`+`date` (labels deliberately NOT the type token, or the naming fix would prove nothing) and leaves the other half bare so the legacy phrasing keeps its coverage, floors the measured actual above zero (a day only lands in `donePlannedMi` if a run with a distance was logged there, so "asked 12, ran exactly 0" is a state the real slice cannot produce), and adds `week-redistributed` + `week-short` to `PLAN_IDS` so the lane invariant covers the two branches that changed most recently.

**VERIFICATION.** Syntax: `planTiers.js`, `coachContext.js`, `coachNarrative.js`, `coachNarrativeSim.js` all `node --check` clean; `LivingPlan.jsx` clean through `esbuild --loader:.jsx=jsx --bundle=false` (101.6kb). Harness sweep, all 19 green, **0 `FAIL` lines of any spelling**: `triad`(53) `tierprog`(108) `adh` `recal` `trace`(45) `allweeks` `rampbase` `deliver` `attrib` `tf3` `misses`(24) `modes`(17) `tracesync`(22) `profilesync`(25) `chips`(4818) `calread`(1651) `prefsync`(65) `coachproj`(54, was 27) and the NEW `triadfwd`(31). `runPlanSim` 2000 plans: `ok: true`, 0 HARD violations, acwr 99.84% / ramp 100.00% / peak 97.26%. `runCoachSim`: 0 violations at seed 20260716 ×6000 **and at seeds 1/42/777/20260101 ×3000 each**. `runQualityEval({nCases:4000})`: 18,186 spoken surfaces, grounded/timely/**concise**/coherent **1.0**, actionable **0.9990**, meanScore **0.9999**, 19 fails — all 19 the pre-existing `actionable: gentle read has no action cue` on strength beats, **0 involving any new branch**; thresholds `actionable ≥ 0.95` / `meanScore ≥ 0.995` met with room. An instrumented probe over the same 6,000 contexts confirms every new path is genuinely exercised rather than dead code behind a green sim: `week-short` 251, `week-redistributed` 85, `week-drift` 3,224 — of which 1,591 name the day ("Thursday's intervals"), 1,633 take the legacy wording, 405 the "and you were ~N mi short" branch, 501 the "which covers part of it" branch, 2,819 the plain causal `so`. **The quality eval caught a real regression in the same round it was written:** `week-short`'s first draft ran ~490 chars and pushed the composed `plan` surface past the 1,000-char wall-of-text bound (`concise` 0.9994) — the exact complaint Emil has made three times. It was trimmed with every fact intact, `concise` is back to 1.0, and `coachproj` now pins the length so a later re-inflation fails in the harness instead of quietly in the eval.

FILES this round — `src/core/planTiers.js` (`TRIAD_PACK_V` → 2, new `refreshTriadForward`), `src/components/LivingPlan.jsx` (import, `refreshCommittedTriad`, the LIVING auto-apply hook), `src/core/coachContext.js` (`donePlannedMi` / `doneUnmeasuredMi` / `weekMiActualRaw`, `label`+`date` on slice entries, `livePlanSlice` carries labels), `src/core/coachNarrative.js` (`nameOf` / `dayNameOf`, gWeekDrift decomposition, the new `week-short` beat), `src/core/sim/coachNarrativeSim.js` (decomposition + label/date generation, PLAN_IDS), `HANDOVER.md`. New harness `/tmp/triadfwd.mjs`; extended `/tmp/coachproj.mjs`; patched `/tmp/tierprog.mjs`, `/tmp/calread.mjs` for the pack-version bump; new diagnostics `/tmp/realweek.mjs`, `/tmp/beatprobe.mjs`, `/tmp/beatprobe2.mjs`.

2026-07-26 (ROUND 95) — **Emil's three ship-blockers closed: the coach stopped narrating the plan back to itself and now reads the miles he actually ran, the calendar tiles stopped going blank on flat weeks (the tier freeze was silently dropping every zero-spread day), and PICK YOUR ROAD collapsed from seven stacked rows into one thin hero line. Build-blind; node-verified by 18 harnesses (6,115 assertions), the 6,000-case coach sim, the 4,000-case quality eval and the 2,000-plan population sim — 0 failures.**

> Resume point for the NEXT window: **(1)** Emil rebuilds and screenshots "Your plan" (web + phone), the month grid, and the coach paragraph — ROUND 95 is NOT build-verified. **(2) NEW AND UNRESOLVED — three different numbers for the SAME week are on screen at once:** the LIVING header says "this week targets 17 mi" (`thisBlockWk.totalWeeklyMiles`, `LivingPlan.jsx` ~1035–1059), the coach says "~21 mi target" (from the planner week), and the week strip says "26 mi" (block week 1). This is exactly the parallel-systems class Emil has said outright he fears; it surfaced during the #133 dig and was deliberately NOT fixed in the same round as three UI/logic fixes. Pick ONE derivation and make the other two read it. **(3)** `toISOString` timezone bug still live in `core/tierFeasibility.js` (~68–72, ~114) and `LivingPlan.jsx`'s session drill-down. **(4)** Orphans to delete: `RecipePath.jsx`, `SeasonPlanGenerator.jsx`, `PlanGeneratorPanel.jsx` — both plan-generator panels write a PARTIAL four-field `planPrefs` object that would wipe `target`/`tier`/`startDate`/`customGoalSecs`/`longRunDow`/`strengthDows`; harmless only because neither is mounted, and now genuinely dangerous because `planPrefs` syncs. **(5)** Web nav labels still swapped (`training`→"EdgeIQ" renders `TrainingTab`; `weekly`→"Trend" renders `EdgeIQ`). **(6)** `goals.weeklyRunDistanceTarget || 30` still powers ~12 dashboard tiles nothing writes back to. **(7)** Reach-rung steepness (`RUNG_ACWR.reach = 1.19`) awaiting Emil's call.

**A) #133 — "why the Coach says 'You didn't get the easy run this week,...' this makes no sense."** It made no sense because it was true of the PLAN and false of HIM. `computePlanSlice` computed the week's projected mileage as `weekMiTarget − missedMi` — a number assembled entirely out of *planned* distances. The only evidence it had about what actually happened was `runOn`, a `Set` of DATES, which carries no distance whatsoever. So a week where Emil skipped a 3-mile Monday and then ran 5.7 / 7.5 / 7.5 instead of 6 / 6 / 6 came out as "tracking ~18 mi against the ~21 target — about 3 mi (14%) light" while he had in fact covered the week. The coach was narrating the plan back to itself and calling it observation. That is the single worst failure mode this app has, because it is the one that makes an honest coach sound like a liar.

**B) The fix — PROJECTED = WHAT YOU HAVE RUN + WHAT IS STILL AHEAD.** `computePlanSlice` (`core/coachContext.js`) gained an optional fifth parameter `runMiOn`: a `Map`/object of `'YYYY-MM-DD'` → miles actually logged. When supplied, `weekMiProjected` becomes `round(max(0, weekMiActual + plannedLeftMi))`; when omitted the old arithmetic survives byte-for-byte, which is load-bearing — `core/coachContext.test.js` calls the 4-arg form at lines 32/45/51/405/426 and the coach sim synthesises the same estimate. `weekMiActual` is set **only** when real miles were supplied; a fabricated `0` would read as "you have run nothing" and would be a worse lie than the one being fixed. A `miCounted` Set guards per-DATE counting so a two-a-day is not counted once per planned session on it. `livePlanSlice` builds the map from `activities` and `workouts` and takes **`Math.max` per date, not the sum** — both stores carry Garmin-synced runs and overlap — and deliberately excludes `dailyLogs.distanceMeters`, which is whole-day step distance, not a run.

**C) The new beat, and the trade it refuses.** `gWeekDrift` (`core/coachNarrative.js`) hoisted `missedKey`/`easyLeft`/`keyLeft`/`missedList` above the gap gate and gained `week-redistributed`: when the athlete missed sessions but the week is nonetheless whole (`weekMiActual != null && gapMi < 2 && !missedKey`), it says so out loud — *"You skipped the easy run but the week is whole — 21 mi run against a ~34 mi week, because you went longer on the days you did get out"* — in a **neutral** tone, and warns that made-up miles can turn an easy day hard. The `!missedKey` gate is the point: 12 easy miles do not replace a tempo, and praising volume that covered for missed intensity is a trade a coach must refuse. A missed key session still falls through to the drift beat or to silence.

**D) #134 — "I still do not see on the calendar tiles the option to run harder or longer."** He was right, and it was not a rendering bug — there was nothing to render. `packTriad` (`core/planTiers.js`) gated the freeze on `!(d.tierSpreadMi > 0)`, dropping every day where all three rungs prescribe the same distance. Emil's near-term weeks are **entirely** flat — the three blocks only diverge after several weeks of ramp — so nothing was frozen for those weeks, so `triadDayFrom` returned `null`, so every tile on the visible part of his calendar was blank. The old gate conflated DISPLAY with RECORD: "three identical numbers are not a choice" is true of display and false of record. A flat day is still a day the athlete was asked for something, and `classifySessionRung` needs the freeze to grade it at all. The spread clause is gone; the `tierMonotone` clause was **deliberately kept**, because three independently generated blocks hit their cut-back weeks at different indices, so a real 7/6/7 row exists and grading a run against a non-ladder would credit "Reach" for a day where Reach asked for less.

**E) The other half of #134 — one honest chip, worded identically on both surfaces.** `TriadRow` in `CalendarTab.jsx` (the single shared component behind both `DayTile` and `MobileDayTile`, per the #130 rule) now detects `spread < 0.05` and renders ONE chip — `all three roads · 4 mi` on web, `all 3 · 4` compact on phone — coloured teal when the logged run covered it, red when short, muted when the day has not happened. Printing `4 / 4 / 4` would paint a choice the athlete does not have; printing nothing reads as a broken feature. The wording and tone mirror `LivingPlan.jsx`'s existing flat-day chip exactly, so the same Tuesday cannot be described two ways on two screens. The tooltip states the whole truth: *"All three roads prescribe the same 4 mi today — there is nothing to choose between them this week."*

**F) #135 — "There is too much space empty to the right. This should be one thin hero line that's it."** He circled roughly half the card. The cause was structural, not cosmetic: PICK YOUR ROAD was a STACK of seven full-width rows (header, numbered step label, chip row, spread-over row, custom-time row, base line, second numbered step + apply button), each holding a left-aligned fragment ~500px wide on a ~1,340px card — so the card was tall AND two-fifths empty, the worst of both. `LivingPlan.jsx` lines 1147–1462 were replaced with ONE wrapping flex row: micro-label, verdict chip, the finish-time chips (now single-line, label + time + peak, with ramp cost moved into the `title`), a divider, the custom-time input + `clear`, a divider, `Spread` + the 3–7 buttons, the short-day warning, a flex spacer, the compact `base · asks · peaks` readout, the apply button and the `working ▾` toggle. It wraps rather than overflows, so the phone gets the same controls in two or three lines. **Nothing was removed** — the numbered step labels became the apply button's own wording (where that instruction actually belongs) and every explanatory paragraph moved behind "show the working", where the rest of the provenance already lived. The file went 2320 → 2291 lines. New `shortestOrdinaryMi` reads the shortest non-long-run, non-race day straight off the block the generator just drew — Emil's *"I see 4 mile run days, for which I am not even going to go out"* — so the warning cannot disagree with the tiles.

**VERIFICATION.** Syntax: `planTiers.js`, `coachContext.js`, `coachNarrative.js`, `coachNarrativeSim.js` all `node --check` clean; `CalendarTab.jsx` and `LivingPlan.jsx` both clean through `esbuild --loader:.jsx=jsx --bundle=false`. Harness sweep, all 18 green, 0 `FAIL:` lines: `triad`(53) `tierprog`(107) `adh` `recal` `trace`(45) `allweeks` `rampbase` `deliver` `attrib` `tf3` `misses`(24) `modes`(17) `tracesync`(22) `profilesync`(25) `chips`(4818) `calread`(1651) `prefsync`(65) and the NEW `coachproj`(27). `runPlanSim` 2000 plans: `ok: true`, 0 HARD violations, acwr 99.84% / ramp 100.00% / peak 97.26%. `runCoachSim({nCases:6000})`: `{seed: 20260716, violationCount: 0}`. `runQualityEval({nCases:4000})`: 17,716 spoken surfaces, grounded/timely/concise/coherent **1.0**, actionable **0.9971**, meanScore **0.9998**, 52 fails — **all 52 are the pre-existing `actionable: gentle read has no action cue` on strength beats, 0 involve the new beat**; `coachQuality.test.js` asserts `actionable ≥ 0.95` and `meanScore ≥ 0.995`, both met. Two harnesses went RED on the `packTriad` change (`tierprog` 2, `calread` 35) because both hard-coded the OLD `tierSpreadMi > 0` rule in their gradeable predicate; both were repointed at the new rule **and given a new positive assertion** (`flatFrozen > 0`) so the change tightened coverage rather than weakening it. An instrumented probe over 6,000 sim cases confirmed 2,836 measured contexts producing 147 `week-redistributed` and 2,659 `week-drift` beats — the new branch is genuinely exercised, not dead code behind a green sim.

FILES this round — `src/core/coachContext.js` (`computePlanSlice` fifth param, `livePlanSlice` max-per-date), `src/core/coachNarrative.js` (`gWeekDrift` + `week-redistributed`), `src/core/planTiers.js` (`packTriad` spread gate removed), `src/components/CalendarTab.jsx` (`TriadRow` flat-day chip), `src/components/LivingPlan.jsx` (hero line, `shortestOrdinaryMi`), `src/core/sim/coachNarrativeSim.js` (measured-week generation), `HANDOVER.md`. New harness `/tmp/coachproj.mjs`; patched `/tmp/tierprog.mjs`, `/tmp/calread.mjs`.

2026-07-25 (ROUND 94) — **Emil's four complaints answered: "Your plan" shrunk and its hierarchy inverted, the tier options moved onto the ACTUAL calendar month tiles (web AND phone), run-days surfaced as a control, and the web-vs-mobile divergence ROOT-CAUSED to two real bugs — `planPrefs` was never in `KEYS` so it never synced, and four surfaces each carried their own finish-time formatter. Build-blind; node-verified by 17 harnesses (6,088 assertions) + the 2000-plan sim, 0 failures.**

> Resume point for the NEXT window: **(1)** Emil rebuilds and screenshots the month grid (web + phone) and "Your plan" on both — ROUND 94 is NOT build-verified. **(2) OPERATIONAL, tell Emil first:** because `planPrefs` has never synced, his two devices currently hold genuinely DIFFERENT plan configs. The migration never clobbers a value already pulled from sync, so after the build lands he should configure the plan ONCE on the device he trusts and let LWW propagate it. **(3)** `toISOString` timezone bug still live in `core/tierFeasibility.js` (~68–72, ~114) and `LivingPlan.jsx`'s session drill-down. **(4)** Orphans to delete: `RecipePath.jsx`, `SeasonPlanGenerator.jsx`, `PlanGeneratorPanel.jsx` — **note both plan-generator panels write a PARTIAL four-field `planPrefs` object that would wipe `target`/`tier`/`startDate`/`customGoalSecs`/`longRunDow`/`strengthDows`; harmless today only because neither is mounted anywhere.** Now that `planPrefs` syncs, that latent write would propagate the damage cross-device — delete them before anything remounts them. **(5)** Web nav labels still swapped (`training`→"EdgeIQ" renders `TrainingTab`; `weekly`→"Trend" renders `EdgeIQ`). **(6)** `goals.weeklyRunDistanceTarget || 30` still powers ~12 dashboard tiles nothing writes back to. **(7)** Reach-rung steepness (`RUNG_ACWR.reach = 1.19`) awaiting Emil's call.

**A) THE #132 ROOT CAUSE — one un-registered key, and every number downstream of it forks.** Emil, with two screenshots side by side: *"the Your plan on web and mobile show two different things."* Web read 19 weeks / starts 27 Jul / committed to **Current** / "Current asks 39 · this plan peaks 39" / ramps 9.3–12.9 %/wk. Mobile read 20 weeks / committed to **Target** / "Target asks 41 · this plan peaks 41" / ramps 7.0–10.1 %/wk. Base agreed at 20.7 on both. It could not be a rendering difference — web and mobile render the **same** `LivingPlan` component — so it had to be data. It was. `LivingPlan` persists the entire shape of the plan with `storage.set('planPrefs', {…})`, and `planPrefs` was **absent from `KEYS`**. `storage.resolveKey` is `KEYS[key] || key`, so it fell through to a bare, unprefixed `planPrefs` localStorage slot; and `cloud-sync.buildSnapshot()` walks `Object.entries(KEYS)`, so **a key not in `KEYS` is a key that has never left the device it was typed on.** `planner` and `planCommitment` ARE in `KEYS` and were syncing fine — which is exactly why the calendar agreed while the plan card did not. Each device therefore held its own `startDate`; a plan starting 27 Jul is 19 weeks to Valencia while one starting a week earlier is 20; a different `weeksToRace` solves a different ramp, which peaks at a different mileage, which makes a different tier reachable. One line, and the two systems computed the same thing differently — the failure this codebase exists to refuse. FIX: `planPrefs: 'arnold:plan-prefs'` registered in `KEYS`, plus `migratePlanPrefsKey()` (modelled on the existing `migrateSupplementKeys` precedent) wired into `Arnold.jsx`'s mount effect. The migration copies **only into an EMPTY slot** — once the key syncs, the second device may already have PULLED the good value, and overwriting that with the phone's stale local copy would resurrect the exact divergence the migration exists to end. The bare key is removed either way so it cannot be read back and re-fork. Plain LWW is the right merge here because this is a settings OBJECT, not a record array.

**B) THE #132 SECOND CAUSE — four formatters, two answers.** Mobile EdgeIQ printed his target as 3:48 and ceiling 3:20; "Your plan" printed 3:49 and 3:21 for the same two numbers. Four surfaces had each written their own h:mm formatter and two of them disagreed about the leftover seconds: `RaceOutlookCard.jsx` (what EdgeIQ actually renders) **floored** the minute, while `LivingPlan.fmtGoal`, `volumeModel.js` and `tierFeasibility.js` **rounded** it. A 3:48:35 goal is "3:48" on one screen and "3:49" on the other, and the athlete is left to guess which one is his goal. NEW canonical `fmtFinish` in `core/time.js` (the zero-import local-date layer, so there is no cycle risk), and all five sites — including the `window.raceOutlookDebug` console formatter, so the console cannot disagree with the screen — now route through it. **It truncates, deliberately.** Rounding 3:48:35 up to "3:49" prints a target 25 seconds EASIER than the one the plan solved for, and every pace derived from the printed figure is correspondingly slower; truncating prints a time that, if you hit it, means you beat the ask. A display error that makes the athlete slightly faster than required is the only direction of error a coach should ship. Truncation also kills a latent bug: `Math.round((3*3600+59*60+45) % 3600 / 60)` is 60, so the old `fmtGoal` rendered a 3:59:45 goal as **"3:60"**.

**C) THE THREE ROADS ARE NOW ON THE CALENDAR ITSELF (#130).** Emil: *"I see nothing on the Calendar for the option on what to run."* He was right and ROUND 93 delivered to the wrong surface — his original ask said "each calendar tile", and the chips had gone into LivingPlan's THIS-WEEK strip. They are now on the month grid, web **and** phone. The load-bearing architectural rule: **the calendar NEVER builds a triad.** `LivingPlan.commitTier()` freezes `packTriad(…)` onto the commitment record at apply time; `CalendarTab` reads it back through `getCommitment()` → `triadDayFrom(packed, weekKey, dayIndex)`. One derivation, one writer, two readers. `TriadRow` is a single shared component consumed by both `DayTile` and `MobileDayTile`, differing only by a `compact` flag, so the two grids are structurally incapable of printing different numbers for the same Tuesday. The phone tile never showed mileage at all before this, so the three targets are new information there, not a re-styling of old.

**D) A CRASH-LEVEL DEFECT CAUGHT BEFORE IT SHIPPED.** The `triadByDate`/`rungLabels` memo lives in `CalendarTab`, but the props were consumed inside `MonthGrid` — which never received them. That is a `ReferenceError` that would have taken down the entire calendar on render. Fixed by threading both through the `<MonthGrid>` call site AND its destructured signature, and by making the consumption defensive (`(triadByDate && triadByDate.get(cell.date)) || null`). Recorded because it is the exact failure mode of build-blind work: esbuild parses the file happily, and only a running render would have found it. A harness that replicates the data path is the only substitute for a build.

**E) "Your plan" shrunk, hierarchy inverted (#129) and run-days surfaced (#131).** *"The Your plan is still big, these tiles are huge, both the week tile and the Pick your road. The Header is small and the Tiles with the time are huge."* The type scale was upside down — the finish-time tiles were the largest thing on the card and the header that says what you are looking at was the smallest. Inverted, on both surfaces. And: *"I see 4 mile run days, for which I am not even going to go out, if I run slower run at 7 miles per run. Maybe I can just adjust how many days to run."* That is a real coaching point, not a preference — a 4-mile day is below the threshold where the trip is worth the shoes for him, and the same weekly volume across fewer days is a legitimately different (and for him better) plan. Run-days is now an explicit control rather than a buried pref.

**F) INVESTIGATED AND DELIBERATELY LEFT ALONE.** `MobileDayTile` computes `headlineMetric`, `hasMobilitySecondary` and `sigImg` and renders none of them — pre-existing dead code, marked `void headlineMetric;` with a comment rather than deleted, to keep the diff to the task. The `packTriad` monotone gate rejects days where the three plans do not form an ascending ladder (11 such days in the real block); that is deliberate and must not be "fixed" — three independently generated blocks hit cut-back weeks at different indices, so 7/6/7 rows are real, and a day that is not a ladder must never be graded as one.

**G) TOOLING NOTES FOR THE NEXT WINDOW.** `device_bash` was dead this entire session ("Workspace unavailable") — **no git from Cowork**; Emil ran `git add/commit/push` himself in `C:\Users\Superuser\Arnold` (repo root is `Arnold`, NOT `arnold-app`). The `Edit` tool requires a prior `Read` of the SAME file *via the Read tool* — `sed`/`cat`/`grep` do not satisfy it (hit twice). `Grep` with `-C` renders some `//` comment lines as a single `/` in its context output; that is a DISPLAY artifact, verified against `Read` — do not "fix" those lines. And the standing trap: harnesses print `   FAIL:` with LEADING SPACES, so **always `grep -c 'FAIL:'`, never `grep -c '^FAIL'`.**

VERIFICATION this round — all nine edited files syntax-clean (`npx esbuild --loader:.jsx=jsx --bundle=false`: `CalendarTab` 129.1kb, `LivingPlan` 101.5kb, `RaceOutlookCard` 17.9kb, `Arnold` 92.3kb, EXIT 0; `node --check` OK on `storage.js`, `time.js`, `volumeModel.js`, `tierFeasibility.js`, `raceOutlookLive.js`). NEW `/tmp/calread.mjs` proves the #130 read-back contract on the REAL frozen pack — every gradeable day round-trips identically through `triadDayFrom` and grades identically under `classifySessionRung` at 6 sweep mileages; every non-gradeable day reads back `null`; the monotone gate rejected 11 non-ladder days; a replica of `indexTriadByDate`'s date walk swept 6 month views asserting each surfaced date decodes back to the week/index it was frozen under, that every weekKey is a Monday, and that no date is served two different days across month views; labels matched by rung never by position; survives JSON + string-key lookup. **1,012 assertions, 0 failed.** NEW `/tmp/prefsync.mjs` proves #132 both ways — `planPrefs` is namespaced and distinct, enters the `buildSnapshot` walk once written, lands on `arnold:plan-prefs` and NOT the bare key, all 12 plan inputs round-trip and `loadPrefs` actually reads each one, `LivingPlan` never touches raw localStorage; four migration cases including **"never overwrite a value already pulled from sync"**; every one of the five formatter files uses `fmtFinish` and none still rounds the minute; 8 explicit cases including Emil's `3:48:35 → "3:48"` and the `3:59:45 → "3:59"` (was "3:60") bug; and an exhaustive sweep of **every second from 2:00:00 to 6:00:00** asserting no 60th minute, never flattering (printed goal never slower than computed), within one minute, and monotone. **65 assertions, 0 failed.** Full regression sweep re-run because `storage.js`/`time.js`/`volumeModel.js`/`tierFeasibility.js` sit on nearly every harness's import path: `triad(53) tierprog(106) adh recal trace(45) allweeks rampbase deliver attrib tf3 misses(24) modes(17) tracesync(22) profilesync(25) chips(4818) calread(1012) prefsync(65)` — **all green, 0 failed.** Population sim `/tmp/runPlanSim.mjs`: 2000 plans, 0 hard violations, ACWR 99.84% / ramp 100% / peak 97.26%.

FILES this round — EDITED: `components/CalendarTab.jsx`, `components/LivingPlan.jsx`, `components/RaceOutlookCard.jsx`, `Arnold.jsx`, `core/storage.js` (NEW `KEYS.planPrefs` + `migratePlanPrefsKey`), `core/time.js` (NEW `fmtFinish`), `core/volumeModel.js`, `core/tierFeasibility.js`, `core/derive/raceOutlookLive.js`, this HANDOVER. NEW harnesses: `/tmp/calread.mjs`, `/tmp/prefsync.mjs` (17 standing harnesses now).

2026-07-25 (ROUND 93) — **The per-session TIER TRIAD shipped end-to-end (feasibility ladder → ramp solver → three parallel blocks → per-day chips → recalibration), the ramp BASE rule rewritten to end the Monday cliff, and a density pass over the two surfaces Emil called "poor design" and "reads like a Bible". Long Cowork session, build-blind (Emil rebuilds on Windows); `npm test` GREEN (838 tests) at the last confirmed build; everything since is node-verified by 15 standing harnesses + a 2000-plan population sim (0 hard violations).**

> Resume point for the NEXT window, in order: **(1)** Emil rebuilds and screenshots the new EdgeIQ Training Profile strip + the calendar tile chips — this round was NOT build-verified. **(2)** The `toISOString` timezone bug still lives in `core/tierFeasibility.js` (lines ~68–72 and ~114) and in `LivingPlan.jsx`'s session drill-down execution score (`dd.toISOString().slice(0,10)`); `core/time.js` is the canonical local-date layer and deliberately never calls it — fix these three to match. **(3)** Orphans to delete: `components/RecipePath.jsx`, `components/SeasonPlanGenerator.jsx` (LivingPlan is now the ONLY generate surface). **(4)** Web nav labels are swapped: the `training` tab labelled "EdgeIQ" renders `TrainingTab`, and `weekly` labelled "Trend" renders `EdgeIQ`. **(5)** The static `goals.weeklyRunDistanceTarget || 30` still powers ~12 dashboard tiles that no plan or commitment ever writes back to — the last "parallel system computing the same thing differently" in the volume story. **(6)** Reach-rung steepness (RUNG_ACWR.reach = 1.19) is awaiting Emil's call.

**A) THE TIER TRIAD — "I need options and I need a plan that provides."** Emil's design, verbatim: every run day should carry three mileage numbers so the athlete picks a road per session rather than per season, and the tier is decided by what he actually runs. NEW `core/planTiers.js` implements it. The load-bearing discovery, measured not assumed (`/tmp/rampsweep.mjs`): a ceiling-driven triad prints the SAME number three times — at the 10%/wk ramp every ceiling from 36 to 51 delivers a peak of 35, because after two support marathons' race+recovery weeks, the every-4th-week cut-backs and the A-race taper, only ~12 progressing weeks survive a 20-week block. **The ceiling never binds; the RAMP STEP binds.** So the triad varies the ramp, and it is safe to do so because Arnold already owns the real limit: a geometric ramp at rate r settles at steady-state ACWR = 1 / [(1 + (1+r)⁻¹ + (1+r)⁻² + (1+r)⁻³)/4], so 10%/wk = ACWR 1.147 against the app's own overreaching line of 1.30 — a large, provably-safe, unused band. `RUNG_ACWR = {baseline 1.15, reach 1.19, challenge 1.25}`, hard-collared `RAMP_HARD_MAX = 0.22` (ACWR 1.315, past the line). `solveRampForPeak()` bisects the smallest ramp that reaches a given peak; `buildTierTriad({rows, committedKey, buildBlock})` builds three real blocks and returns rungs carrying `{rung, tierKey, tierLabel, goalSecs, needsPeakMi, rampPct, acwr, band, reached, refused, deliversPeakMi, why, weeks}`; `mergeTriadWeeks(rungs)` zips them per day into `{tiers:{baseline,reach,challenge}, tierable, tierSpreadMi, tierDeltaMi, tierMonotone}`. This also CORRECTS something the codebase told Emil in good faith and got wrong: `planGenerator.js` concluded that racing Berlin + NYC "costs sixteen mi/wk of peak" — that was measured by deleting the races while holding the ramp at 10%, which conflates two different levers.

**B) `classifySessionRung` — the tier is DERIVED from the run, never asked.** `classifySessionRung({tiers, actualMi})` → `{rung, short, targetMi, actualMi}`, tolerance `max(0.3, target·4%)`. `rung null + short true` = ran, but under baseline. `rung null + short false` = nothing to judge. Feeds `tierHitRate()` (`REBASE_HIT_RATE = 0.70` over `REBASE_WEEKS = 4` — Emil's numbers verbatim) and `DEMOTE_HIT_RATE = 0.50`, which drive the recalibration card. Demotion stays ADVISORY: it offers, it never rewrites the calendar (task #123).

**C) The RAMP BASE rule, and the Monday cliff.** Emil: *"re-starting the plan with a 20-mile peak, especially if I run a 33-mile week this current week, is a waste of ramp-up period. Is this a rule based on something else apart from the last-28-day average?"* It was — and the fix he chose (via question) is now in `rampBaseMi` (`core/seasonCoach.js`): the anchor is `max(median-of-4-complete-weeks, MIN-of-the-last-two)`. **Min, not mean** — the mean fails the hero-week test outright (one 60-mile week against a 19-mile history gives 39.7 and the base leaps to the demonstrated cap on the strength of one Sunday); the min asks "what level do the last two weeks BOTH support", which is the definition of a level rather than an event. On top of that, the **live week-to-date counts as a FLOOR ONLY, collared to one 10% step above the anchor** — the same MAX_RAMP_PCT the rest of the app ramps at. That collar is what kills the Monday cliff: a free floor would jump the base on Saturday and then CLAW IT BACK on Monday when the week completed and minLast2 judged it, which is precisely the same-thing-computed-two-ways jitter that makes a living plan untrustworthy. With the collar the number is monotone across the boundary — 19.3 → 20.9 Saturday → 21.2 Monday (`/tmp/living.mjs`, replaying the real chain at three timestamps). **Consequence for Emil, and it answers his direct question: waiting until Sunday night to regenerate is NO LONGER necessary. Any day is safe.** `recentRunStats` is deliberately UNCHANGED — chronic load stays the 28-day mean.

**D) Race weeks now obey Emil's two rules.** (1) A race shorter than a half marathon is just that day's scheduled run and counts toward weekly mileage; (2) a half or longer BECOMES that week's long run (`longRunIsRace`) instead of the long run being cut beside it. Weeks carry both `targetWeeklyMiles` (the training ramp line, race excluded) and `totalWeeklyMiles` (what you will actually cover) — the two genuinely differ on a race week and both are true, so the UI prints the total with the training line beside it rather than silently picking one.

**E) LivingPlan is now the ONLY generate surface.** Emil's call. `PICK YOUR ROAD` lists every option including the unreachable ones with the limiter named (hiding an unreachable goal hides the reason). Selecting redraws the PREVIEW only; the calendar changes on apply. The recalibration card offers rebuild-lower AND keep-the-goal-change-the-lever, never only "aim lower".

**F) DENSITY PASS (Emil, this round, with 7 screenshots).** *"Fix the drop down on EdgeIQ, it takes too much space when not unfolded, then when unfolded the information is bunched up with a ton of empty space… the Your Plan tab reads like a Bible… way too much narrative for web and mobile."* Both diagnosed as layout, not content. **`RaceOutlookCard.jsx`** (the ONE shared TRAINING PROFILE card — `EdgeIQ.jsx`, `TrainingTab.jsx` and `MobileHome.jsx` all render it, so one fix covers three surfaces): collapsed strip went from three stacked rows to a single wrapping line; the expanded panel became a two-column `auto-fit minmax(300px,1fr)` grid so the ladder/trace column and the other-races column share the width instead of the content hugging the left half; `LadderGrid` bins were capped at `maxWidth:128` (five bins in ~700px of a ~1400px card) → now `auto-fit minmax(84px,1fr)`; `Moon` went from a full-width flex bar per race (7 races = 7 near-empty bars) to a tiled chip in `auto-fill minmax(190px,1fr)`; every prose paragraph became a labelled `Fact` chip — **the numbers all survive, the sentences around them do not, except where a sentence tells the athlete to DO something.** **`LivingPlan.jsx`**: the "Bible" is now a one-line headline (`Base N mi/wk · <Tier> asks N · this plan peaks N`) plus a **`show the working ▾`** disclosure, closed by default on every surface, holding `cur.why`, the ramp-cost explanation, the delivery check, the base provenance read and the not-advised list — **unchanged and in full**. Deleting them would have traded one complaint for a worse one: a number with no provenance is a number Emil cannot argue with. The `noBase` box, the `goalVol` clamp note and the recalibration card's two "this only redraws the preview" paragraphs were each cut to one sentence.

**G) CALENDAR TILE CHIPS (Emil: "each tile will show the target distance for Current, Target, Stretch and Goal as separate mile numbers… and when the run is complete only that number illuminates as done").** The reason he had never SEEN the triad row is that the old gate killed it on `status === 'done' || 'missed'` — exactly when he wanted it — and blanked it whenever `tierSpreadMi === 0` (week 1 and every cut-back week). The gate now changes **authorship** rather than visibility: before the run the chips are tappable and price the week; once the day is settled they become a verdict, `classifySessionRung` derives which rung the run actually hit, that ONE chip lights and the others drop to `opacity 0.34`; a run under all three lights nothing and marks the lowest rung red; a flat week shows one honest `all three roads · N mi` chip instead of three identical numbers. Chips are labelled with the athlete's OWN vocabulary — Current / Target / Stretch / Ceiling / Goal / "Your time", pulled from each rung's `tierLabel` and **matched by `rung`, never by array position**, so a triad that ever ships fewer than three rungs cannot silently shift every label one place along. On mobile the labels drop and the week-budget row (now colour-dotted and labelled with the same names) is the legend.

VERIFICATION this round — `npx vitest run` is unusable in the Cowork sandbox (`Cannot find package 'vite'`), so the standing harnesses are hand-written assertion sweeps under plain node, run with cwd `arnold-app`: `triad, tierprog, adh, recal, trace, allweeks, rampbase, deliver, attrib, tf3, misses, modes, tracesync, profilesync` — **all green**. NEW `/tmp/chips.mjs` proves the tile-chip contract on the REAL triad: 1,416 logged-mileage values swept across 48 tierable days (34 flat days take the "all three roads" chip) — never two chips lit, running more never demotes the rung, the exact prescribed distance lights exactly its own chip, zero logged lights nothing and marks nothing short, labels are distinct, and dropping the middle rung does not shift the other two. **4,818 assertions, 0 failed.** Population sim `/tmp/runPlanSim.mjs`: 2000 plans, 0 hard violations, ACWR 99.84% / ramp 100% / peak 97.26%. **HARNESS GOTCHA — the harnesses print `   FAIL:` with LEADING SPACES, so `grep -c '^FAIL'` returns 0 and produces a FALSE ALL-GREEN. Always `grep -c 'FAIL:'`.** Both edited components esbuild-clean (`RaceOutlookCard` 17.9kb, `LivingPlan` 99.2kb, EXIT 0).

FILES this round — NEW: `core/planTiers.js`, `core/tierFeasibility.js`, `components/RaceOutlookCard.jsx`. EDITED: `components/LivingPlan.jsx`, `components/EdgeIQ.jsx`, `components/TrainingTab.jsx`, `components/MobileHome.jsx`, `core/seasonCoach.js` (`rampBaseMi`), `core/hub/planGenerator.js` (race-week rules), `core/derive/raceOutlookLive.js` (trace), this HANDOVER. **NOTE: `device_bash` was unavailable for this whole session ("Workspace unavailable — the isolated Linux environment on this device failed to start"), so NO git command could be run from Cowork. Files were written to the device via `device_commit_files`; Emil ran `git add/commit/push` himself in `C:\Users\Superuser\Arnold`.**
...
**3.1 COMPLETE (2026-07-03):** 3.1a assembler + 3.1b conflicts + **3.1c** (A-race designation reuses the existing race
A/B/C priority — no new UI; `GoalConflicts.jsx` conflict card mounted in GoalsHub, top of tab: shows both trade-off
directions + tap-to-choose → `setGoalResolution`) + **3.1d** (`goalModelSim.test.js`: 5,000 synthetic goal-sets, model
always well-formed). Deferred (minor): training-goal deadline *editor fields* (model treats training as ongoing = correct).
**Observability:** `__arnoldDiag()` now prints a `learning` block — `learning.sweat.{rateLhr,n,confidence}` + learned
